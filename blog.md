...
<section class="py-16 md:py-20 lg:py-24"><div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3">{% for blog in site.data.blogs %}<article id="{{ blog.slug }}" class="overflow-hidden rounded-2xl bg-white shadow-md transition hover:-translate-y-1 hover:shadow-xl"><img class="aspect-video w-full object-cover" src="{{ blog.image }}" alt="{{ blog.title }}" loading="lazy"><div class="p-6"><span class="text-sm font-bold text-primary">{{ blog.date }}</span><h2 class="mt-3 font-heading text-xl font-bold text-primary">{{ blog.title }}</h2><p class="mt-3 text-sm leading-7 text-textGray">{{ blog.excerpt }}</p><p class="mt-3 text-sm leading-7 text-textGray">At Inspiritation Classes, students are guided to convert preparation time into measurable progress through planning, revision, and regular test analysis.</p></div></article>{% endfor %}</div></section>
